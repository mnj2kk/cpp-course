...
inline unsigned char * encode_algorithmic_indicator(unsigned char * start, bool value)
{
    *start++ = (value ? 'Y' : 'N');
    return start;
}

inline unsigned char * encode_bitfield(unsigned char * start, const std::vector<uint8_t> & bitfield)
{
    start = encode(start, static_cast<uint8_t>(bitfield.size()));
    for (std::size_t i = 0; i < bitfield.size(); ++i) {
        start = encode(start, bitfield[i]);
    }
    return start;
}
